...
export default function MessageContainer() {
  ...
  const getChannelMessages = async () => {
    try {
      ...
      if (response.data.channel.messages) {
        setSelectedChatMessages(response.data.channel.messages)
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const checkIfImage = filePath => {
    const imageRegex = /\.(jpg|jpeg|png|gif|bmp|tiff|tif|svg|ico|heic|heif)$/i
  ...
  const renderChannelMessages = (message) => {
    console.log(message.sender._id !== userInfo.id ? "text-left" : "text-right");
    return (
